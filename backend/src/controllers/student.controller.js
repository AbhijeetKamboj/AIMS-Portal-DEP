...
export const getTranscript = async (req, res) => {
    try {
        ...
        (transcript || []).forEach(row => {
            ...
            // Calculate Stats
            // Include in GPA if grade exists and is not W (Withdrawn) or P (Pass - assuming P is not in grade_scale or has specific logic)
            // Typically P/NP courses don't count for GPA but count for earned credits.
            // Our grade_scale has A, B, etc. and F. W=0.

            if (row.grade && gradeMap[row.grade] !== undefined && row.grade !== 'W') {
            ...
            // Registered Credits (all except withdrawn?)
