...
export default function FacultyApprovals() {
    ...
    const handleBulkApprove = async (status) => {
        ...
        try {
            ...
            toast.success(`${selectedArray.length} request(s) ${status === 'pending_advisor' ? 'approved' : 'rejected'}!`);
            setSelectedRequests(new Set());
            fetchRequests();
        } catch (err) {
            toast.error(err.message);
                ...
                <>
                    {selectedRequests.size > 0 && (
                        <div className="px-8 py-4 bg-blue-50 border-b border-blue-100 flex justify-between items-center">
                            ...
                            <div className="flex gap-3">
                                <button
                                    onClick={() => handleBulkApprove('pending_advisor')}
                                    disabled={loading}
                    ...
                    <div className="p-8 space-y-6">
                        {Object.values(grouped).map((group) => (
                            <div key={group.offeringId} className="border border-gray-200 rounded-xl p-6 bg-white hover:shadow-soft transition-shadow">
                                <div className="mb-4">
                                    <h4 className="font-bold text-lg text-gray-900">{group.courseCode}</h4>
                                </div>

                                <div className="space-y-3">
                                    {group.requests.map((req, idx) => {
                                        const globalIdx = requests.indexOf(req);
                                        const requestKey = globalIdx.toString();
                                            ...
                                            <div key={idx} className={`flex items-center gap-4 p-4 rounded-lg border transition-colors ${isSelected ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-100 hover:border-gray-200'}`}>
                                                ...
                                                <div className="flex gap-2 flex-shrink-0">
                                                    <button
                                                        ...
                                                        disabled={loading}
                                                        className="px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-lg text-xs font-bold hover:bg-emerald-200 transition"
                                                    >
                                                        Approve
                                                    </button>
                                                    <button
                                                        ...
                                                        disabled={loading}
                                                        className="px-3 py-1.5 bg-red-100 text-red-700 rounded-lg text-xs font-bold hover:bg-red-200 transition"
                                                    >
                                                        Reject
                                                    </button>
                                                </div>
