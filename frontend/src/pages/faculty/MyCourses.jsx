        ...
        <div className="w-full">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex p-0.5 bg-gray-100 rounded-lg">
                        <button
                            ...
                            className={`px-3 py-1 text-sm font-bold rounded-md transition-all ${viewTab === "active" ? "bg-white text-black shadow-sm" : "text-gray-500"}`}
                        >
                            Active ({activeCourses.length})
                        </button>
                        <button
                            onClick={() => setViewTab("completed")}
                            className={`px-3 py-1 text-sm font-bold rounded-md transition-all ${viewTab === "completed" ? "bg-white text-black shadow-sm" : "text-gray-500"}`}
                        >
            ...
            <div className="space-y-4">
                ...
                {filteredCourses.map(course => (
                    <div key={course.id} className={`group border rounded-xl p-6 transition-all duration-200 bg-white ${course.status === 'completed' ? 'border-gray-300 bg-gray-50' : 'border-gray-200 hover:shadow-medium hover:border-gray-300'}`}>
                        <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-6">
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-1 flex items-center gap-1">
                                    <span>🗓️</span> {course.semesters.name}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-3">
                            <button
                                onClick={() => setActiveModal({ type: 'announcements', offering: course })}
                            ...
                            <button
                                onClick={() => setActiveModal({ type: 'materials', offering: course })}
                                className="flex-1 min-w-[140px] px-4 py-2.5 bg-gray-50 text-gray-900 border border-gray-200 rounded-lg text-sm font-semibold hover:bg-gray-100 hover:border-gray-300 transition flex items-center justify-center gap-2 group/btn"
                            >
                                <span className="group-hover/btn:scale-110 transition-transform">📚</span> Materials
                            </button>
                            <button
                                onClick={() => setActiveModal({ type: 'enrollments', offering: course })}
                                className="flex-1 min-w-[140px] px-4 py-2.5 bg-gray-50 text-gray-900 border border-gray-200 rounded-lg text-sm font-semibold hover:bg-gray-100 hover:border-gray-300 transition flex items-center justify-center gap-2 group/btn"
                            >
                            ...
                            {course.status === 'approved' && (
                                <button
                                    onClick={() => setActiveModal({ type: 'grades', offering: course })}
                                    className="flex-1 min-w-[140px] px-4 py-2.5 bg-gray-50 text-gray-900 border border-gray-200 rounded-lg text-sm font-semibold hover:bg-gray-100 hover:border-gray-300 transition flex items-center justify-center gap-2 group/btn"
                                >
