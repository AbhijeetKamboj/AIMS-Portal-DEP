...
export default function FacultyBulkEnroll() {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            setRollNumbersInput(rollNumbers.join('\n'));
            toast.success(`Loaded ${rollNumbers.length} roll numbers from file`);
        } catch (error) {
            ...
            <div className="mb-6">
                <div className="flex p-1 bg-gray-100 rounded-xl w-fit mb-6">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${inputMethod === "manual" ? "bg-white text-black shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
                    >
                        Manual Entry
                    </button>
                    <button
                        onClick={() => setInputMethod("file")}
                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${inputMethod === "file" ? "bg-white text-black shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
                    >
                        Upload File
                    </button>
                </div>
                    ...
                    <div className="space-y-4">
                        <div className="relative">
                            ...
                            <label
                                ...
                                className="flex flex-col items-center justify-center w-full p-8 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors"
                            >
                                <svg className="w-10 h-10 text-gray-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                </svg>
                                <p className="text-sm font-medium text-gray-700">Upload CSV, TXT, or JSON file</p>
                                <p className="text-xs text-gray-500 mt-1">One roll number per line or comma-separated</p>
                            </label>
                        </div>
                        ...
                        {rollNumbersInput && (
                            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                <p className="text-sm text-blue-900 font-medium">
                                    {rollNumbersInput.split('\n').filter(r => r.trim()).length} roll numbers loaded
                                </p>
                            </div>
                        )}
                    </div>
