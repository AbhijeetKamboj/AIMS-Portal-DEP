...
import CourseInfoModal from "../../components/CourseInfoModal.jsx";
...
export default function Offerings() {
    ...
    useEffect(() => {
        ...
        // Status Filter
        if (statusFilter !== "all") {
            console.log("Filtering with statusFilter:", statusFilter);
            console.log("Available enrollments map:", myEnrollments);
            console.log("Data before filter:", data.map(o => ({ id: o.id, code: o.courses.course_code, status: myEnrollments[o.id] })));
            ...
            res = res.filter(o => {
                ...
                if (statusFilter === "available") {
                    // Available means no enrollment at all (including completed, backlog, etc.)
                    const isAvailable = !status;
                    console.log(`Course ${o.courses.course_code} (id: ${o.id}): status=${status}, isAvailable=${isAvailable}`);
                    return isAvailable;
                }
                return true;
            });
            console.log("Data after filter:", res.length);
        }
    ...
    const fetchData = async () => {
        ...
        if (!offerings.error) {
            ...
            console.error("Error fetching offerings:", offerings.error);
        }

        if (!enrolls.error && Array.isArray(enrolls)) {
            const map = {};
            enrolls.forEach(e => {
                // Mark as completed if semester is locked/completed
                if (e.semester_status === 'locked' || e.semester_status === 'completed') {
                    map[e.offering_id] = 'completed';
                } else {
                    // Use the actual status (enrolled, pending_faculty, pending_advisor, rejected, etc.)
                    map[e.offering_id] = e.status;
                }
            });
            setMyEnrollments(map);
            console.log("Enrollments fetched and mapped:", map);
        } else {
            console.error("Error fetching enrollments or invalid format:", enrolls?.error || "Not an array");
            setMyEnrollments({});
        }
        setLoading(false);
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-2">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    ...
                    <div className="relative w-full sm:w-64">
                        <input
                            type="text"
                            placeholder="Search code, name, professor..."
                            value={searchQuery}
            ...
            <div className="grid gap-4">
                {loading ? (
                    ...
                    filteredData.map(o => {
                        ...
                        if (status === 'completed') {
                             statusBadge = <span className="px-3 py-1 bg-blue-100 text-blue-800 text-xs font-bold uppercase rounded-full tracking-wide">Completed</span>;
                             actionBtn = (
                                 <button disabled className="px-6 py-2.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-lg text-sm font-bold opacity-75 cursor-default">
                                     Completed
                                 </button>
                             );
                             ...
                             statusBadge = <span className="px-3 py-1 bg-red-100 text-red-800 text-xs font-bold uppercase rounded-full tracking-wide">Backlog</span>;
                             actionBtn = (
                                 <button disabled className="px-6 py-2.5 bg-red-100 text-red-800 border border-red-300 rounded-lg text-sm font-bold opacity-90 cursor-default">
                                     Backlog
                                 </button>
                             );
                ...
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn" onClick={() => setSelectedOffering(null)}>
                    <div className="bg-white rounded-2xl shadow-strong max-w-md w-full p-6 animate-scaleIn" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setSelectedOffering(null)} className="text-gray-400 hover:text-gray-600 p-1 cursor-pointer">
