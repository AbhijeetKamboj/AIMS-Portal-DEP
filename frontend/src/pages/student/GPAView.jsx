            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-gradient-to-br from-teal-700 to-teal-800 text-white rounded-2xl p-6 shadow-lg relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        <div className="flex items-start justify-between">
                            <div>
                                <p className="text-teal-200 text-xs font-semibold uppercase tracking-wider mb-2">Total Credits</p>
                                <div className="text-5xl font-bold tracking-tight">{cgpa?.total_credits || 0}</div>
                            </div>
                ...
                {!transcript?.semesters || transcript.semesters.length === 0 ? (
                    ...
                    <div className="space-y-5">
                        {transcript.semesters.map((sem, idx) => (
                            <div key={idx} className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300">
                                {/* Semester Header - Clean Design */}
                                <div className="bg-gradient-to-r from-gray-50 to-white px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                    ...
                                    <div className="w-12 h-12 bg-slate-200 rounded-xl flex items-center justify-center">
                                        ...
                                        <div>
                                            <h4 className="text-lg font-bold text-gray-900">{sem.semester_name}</h4>
                                            <p className="text-sm text-gray-500 mt-0.5">{sem.courses?.length || 0} courses</p>
                                        </div>
                                    </div>
                                    <div className="flex gap-3 w-full sm:w-auto">
                                        <div className="flex-1 sm:flex-none bg-slate-100 px-4 py-2.5 rounded-xl text-center border border-slate-200">
                                            <p className="text-xs text-slate-700 uppercase font-bold tracking-wider">SGPA</p>
                                            <p className="text-2xl font-bold text-slate-900">{sem.sgpa}</p>
                                        </div>
                                        <div className="flex-1 sm:flex-none bg-slate-100 px-4 py-2.5 rounded-xl text-center border border-slate-200">
                                            <p className="text-xs text-slate-700 uppercase font-bold tracking-wider">Credits</p>
                                            <p className="text-2xl font-bold text-slate-900">{sem.credits}</p>
                                        </div>
                                        <div className="flex-1 sm:flex-none bg-zinc-100 px-4 py-2.5 rounded-xl text-center border border-zinc-200">
                                            <p className="text-xs text-zinc-700 uppercase font-bold tracking-wider">CGPA</p>
                                            <p className="text-2xl font-bold text-zinc-900">{sem.cgpa}</p>
                                        </div>
                                    </div>
                                </div>
